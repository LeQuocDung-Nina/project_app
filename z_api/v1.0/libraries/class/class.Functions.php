...
class Functions
{
    ...
    /* Validate member's info */
    public function validateInfoMember($data)
    {
        if (!empty($data)) {
            $dataValidate = [];
            ...
            unset($dataValidate["login_session"]);
            unset($dataValidate["id"]);

            $detailMember = $this->d->rawQueryOne("select id from #_member where username = ? and email = ? limit 0,1", array($dataValidate['username'], $dataValidate['email']));

            if (!empty($detailMember)) {
                $this->d->where('id', $detailMember["id"]);
                $this->d->update('member', $dataValidate);
            } else {
                $this->d->insert('member', $dataValidate);
            }
        }
    }
